...
public class VentaControlDao extends DaoImplement<Venta>{
   ...
   private int particion(Venta[] arr, int inicio, int fin, String field, Integer tipo) {
    ...
    /*public DynamicList<Venta> buscar(String texto, DynamicList<Venta> ventaList, String field) {
        DynamicList<Venta> lista = new DynamicList<>();
        try {
            Venta[] aux = shellSort(ventaList, field, 0).toArray();
            for (Venta venta : aux) {
                Field attribute = Utiles.getField(Venta.class, field);
                if (attribute != null) {
                    ...
                    // Verifica si el valor del campo actual o de los campos asociados contiene el texto
            ...
            while (inicio <= fin) {
                ...
                if (attribute != null) {
                    ...
                    if (fieldValue != null && getFieldValue(fieldValue, field).toLowerCase().contains(texto.toLowerCase())) {
                        // Se encontró el elemento
                        resultados.add(venta);
                        // Recorrer hacia atrás para incluir todos los resultados
                        ...
                        //Recorrer hacia adelante para incluir todos los resultados
    ...
    //busqueda lineal binaria
    /*public DynamicList<Venta> buscarMixto(String texto, DynamicList<Venta> ventaList, String field) {
        DynamicList<Venta> resultados = new DynamicList<>();
        try {
            DynamicList<Venta> listaOrdenada = shellSort(ventaList, field, 0);
            Venta[] aux = listaOrdenada.toArray();

            // Búsqueda lineal para encontrar una región probable
            DynamicList<Venta> regionProbable = new DynamicList<>();
            for (Venta venta : aux) {
                Field attribute = Utiles.getField(Venta.class, field);
                if (attribute != null) {
                    attribute.setAccessible(true);
                    Object fieldValue = attribute.get(venta);
                    if (fieldValue != null && getFieldValue(fieldValue, field).toLowerCase().contains(texto.toLowerCase())) {
                        regionProbable.add(venta);
                    }
                }
            }
            // Si la región probable está vacía, no hay coincidencias
            if (regionProbable.isEmpty()) {
                return resultados;
            }
            // Aplicar búsqueda binaria en la región probable
            DynamicList<Venta> regionOrdenada = shellSort(regionProbable, field, 0);
            Venta[] regionArray = regionOrdenada.toArray();

            int inicio = 0;
            int fin = regionArray.length - 1;

            while (inicio <= fin) {
                int medio = inicio + (fin - inicio) / 2;
                Venta venta = regionArray[medio];
                Field attribute = Utiles.getField(Venta.class, field);
                if (attribute != null) {
                    attribute.setAccessible(true);
                    Object fieldValue = attribute.get(venta);
                    if (fieldValue != null && getFieldValue(fieldValue, field).toLowerCase().contains(texto.toLowerCase())) {
                        ...
                        // Recorrer hacia atrás para incluir todos los resultados
                        for (int i = medio - 1; i >= 0; i--) {
                            Venta ventaAnterior = regionArray[i];
                            if (getFieldValue(ventaAnterior, field).toLowerCase().contains(texto.toLowerCase())) {
                                resultados.add(ventaAnterior);
                            }
                        }
                        // Recorrer hacia adelante para incluir todos los resultados
                        for (int i = medio + 1; i < regionArray.length; i++) {
                            Venta ventaSiguiente = regionArray[i];
                            if (getFieldValue(ventaSiguiente, field).toLowerCase().contains(texto.toLowerCase())) {
                                resultados.add(ventaSiguiente);
                            }
                        }
            ...
            System.out.println("Error en buscarMixto " + e.getMessage());
        }
        return resultados;
    }*/
    ...
    public static void main(String[] args) throws EmptyException {
        VentaControlDao ventaControlDao = new VentaControlDao();
        DynamicList<Venta> ventas = ventaControlDao.getVentas();

        System.out.println("Busqueda Binaria");
        System.out.println(ventaControlDao.busquedaBinaria("q", ventas, "destino").toString());
        //System.out.println("Buscar");
        //System.out.println(ventaControlDao.buscar("q", ventas, "destino").toString());
        System.out.println("Busqueda lineal binaria");
        System.out.println(ventaControlDao.busquedaLinealBinaria("q", ventas, "destino"));
